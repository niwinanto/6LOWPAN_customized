...
static void gen_mesh_header(int src_mode,int dst_mode){
	...
	if(!dst_mode){memcpy(mesh_header.addr+mesh_size,eui_64,8);mesh_size+=8;}
	else{memcpy(mesh_header.addr+mesh_size,eui_64,2);mesh_size+=2;}
...
static void gen_packet(struct sk_buff *skb,unsigned char *data){
	...
	frag_header.dgram_size = (unsigned char *)skb->tail - data;
	frag_header.dgram_tag = 1;
	...
	if(HC1encoded.hc2){
		if(HC1encoded.nh==1) {memcpy(packet+size,&udphdr,udp_size); size += udp_size;}
...
static void tcp_hc2(struct tcphdr *tcp_header){}
